...
test_output_formats() {
    ...
    echo -e "${YELLOW}Testing security kms list --output json...${NC}"
    JSON_OUTPUT=$($ACLOUD_CMD security kms list --project-id "$PROJECT_ID" --output json 2>&1)
    JSON_EXIT=$?
    if [ $JSON_EXIT -ne 0 ]; then
        echo -e "${RED}✗ security kms list --output json: command failed (exit $JSON_EXIT)${NC}"
    elif echo "$JSON_OUTPUT" | grep -qF "No "; then
        echo -e "${YELLOW}⚠ security kms list --output json: no resources — format validation skipped${NC}"
    elif is_valid_json "$JSON_OUTPUT"; then
        echo -e "${GREEN}✓ security kms list --output json: valid JSON${NC}"
    else
        echo -e "${RED}✗ security kms list --output json: output is not valid JSON${NC}"
    fi

    echo -e "${YELLOW}Testing security kms list --output yaml...${NC}"
    YAML_OUTPUT=$($ACLOUD_CMD security kms list --project-id "$PROJECT_ID" --output yaml 2>&1)
    YAML_EXIT=$?
    if [ $YAML_EXIT -ne 0 ]; then
        echo -e "${RED}✗ security kms list --output yaml: command failed (exit $YAML_EXIT)${NC}"
    elif echo "$YAML_OUTPUT" | grep -qF "No "; then
        echo -e "${YELLOW}⚠ security kms list --output yaml: no resources — format validation skipped${NC}"
    elif echo "$YAML_OUTPUT" | grep -qE '^[a-zA-Z].*:|^- '; then
        echo -e "${GREEN}✓ security kms list --output yaml: output looks like YAML${NC}"
    else
        echo -e "${RED}✗ security kms list --output yaml: output does not look like YAML${NC}"
    fi
    echo ""
}
